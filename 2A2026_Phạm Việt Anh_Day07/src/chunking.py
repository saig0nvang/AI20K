...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        # Cleanup: remove empty strings and strip whitespace
...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if not remaining_separators:
            # Last resort: if no more separators, hard split by chunk_size
            return [
                current_text[i : i + self.chunk_size]
                for i in range(0, len(current_text), self.chunk_size)
            ]

        # Take the current separator and prepare for recursion
        sep = remaining_separators[0]
        next_seps = remaining_separators[1:]

        # If separator is not found, try the next one
        ...
        # Logic to split and then optionally group small parts
        for i, part in enumerate(parts):
            # Re-add the separator (except for the very last part if it wasn't at the end)
            is_last = i == len(parts) - 1
            piece = part + (sep if not is_last else "")
            ...
            if len(piece) > self.chunk_size:
                ...
                # Group small pieces into a single chunk
